...
/// @nodoc
mixin _$CityState {
  List<CityModel> get items => throw _privateConstructorUsedError;
...
abstract class $CityStateCopyWith<$Res> {
  ...
  @useResult
  $Res call({List<CityModel> items, String? error, bool isLoading});
}
...
class _$CityStateImpl implements _CityState {
  const _$CityStateImpl(
      {final List<CityModel> items = const [],
      this.error,
  ...
  @override
  String toString() {
    return 'CityState(items: $items, error: $error, isLoading: $isLoading)';
  }
...
abstract class _CityState implements CityState {
  const factory _CityState(
      ...
      final String? error,
      final bool isLoading}) = _$CityStateImpl;
