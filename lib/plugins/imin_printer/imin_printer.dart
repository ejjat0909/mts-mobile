...
class IminPrinter {
  ...
  Future<void> printSingleBitmap(
    ...
    return IminPrinterPlatform.instance.printSingleBitmap(
      img,
  ...
  Future<void> printMultiBitmap(
    ...
    return IminPrinterPlatform.instance.printMultiBitmap(
      imgs,
      pictureStyle: pictureStyle,
    );
  }
  ...
  /// ```
  /// {@end-tool}
  Future<void> printSingleBitmapBlackWhite(
    dynamic img, {
  ...
  Future<void> printQrCode(String data, {IminQrCodeStyle? qrCodeStyle}) {
    return IminPrinterPlatform.instance.printQrCode(
      data,
  ...
  Future<void> printBarCode(
    ...
    return IminPrinterPlatform.instance.printBarCode(
      barCodeType,
      barCodeContent,
      style: style,
    );
  ...
  /// ```
  /// {@end-tool}
  Future<void> printBarCodeToBitmapFormat(
    String barCodeContent,
    int width,
    int height,
    IminBarCodeToBitmapFormat codeFormat,
  ) {
    return IminPrinterPlatform.instance.printBarCodeToBitmapFormat(
      barCodeContent,
      width,
      height,
      codeFormat,
  ...
  /// Detects if the connected printer is an Imin printer
  /// Returns true if it's an Imin printer, false if it's not, and null if detection failed
  Future<bool?> isIminPrinter() async {
    try {
      // First try to get the printer model name
      final modelName = await getPrinterModelName();
      if (modelName != null) {
        // If we got a model name, it's likely an Imin printer
        // Check if the model name contains "imin" (case insensitive)
        return modelName.toLowerCase().contains('imin');
      }

      // If model name is null, try to get the print model
      final printModel = await getPrintModel();
      if (printModel != null && printModel != 'Unknown Model') {
        // If we got a print model and it's not the default "Unknown Model",
        // it's likely an Imin printer
        return true;
      }

      // Try to get the SDK version as a final check
      final sdkVersion = await getSdkVersion();
      if (sdkVersion != null) {
        // If we can get the SDK version, it's likely an Imin printer
        return true;
      }

      // If all checks fail but no exceptions were thrown,
      // it's likely not an Imin printer
      return false;
    } catch (e) {
      prints('Error detecting Imin printer: $e');
      return null; // Detection failed
    }
  }

  /// Detects if the connected printer is a built-in printer
  /// Returns true if it's a built-in printer, false if it's not, and null if detection failed
  Future<bool?> isBuiltInPrinter() async {
    try {
      // First check if it's an Imin printer
      final isImin = await isIminPrinter();
      if (isImin == null) {
        return null; // Detection failed
      }

      if (!isImin) {
        return false; // Not an Imin printer, so not built-in
      }

      // For Imin printers, try to determine if it's built-in based on model name
      final modelName = await getPrinterModelName();
      if (modelName != null) {
        // Check for keywords that might indicate a built-in printer
        // This is a heuristic and may need adjustment based on actual model names
        final lowerModelName = modelName.toLowerCase();
        return lowerModelName.contains('built') ||
            lowerModelName.contains('internal') ||
            lowerModelName.contains('embedded');
      }

      // If we can't determine from the model name, check if we can initialize the printer
      // Built-in printers should be easier to initialize
      final initResult = await initPrinter();
      if (initResult == true) {
        // Successfully initialized, likely a built-in printer
        return true;
      }
      ...
      prints('Error detecting built-in printer: $e');
      return null; // Detection failed
    }
  }
  ...
  /// Gets comprehensive information about the connected printer
  /// Returns a map with various printer details
  Future<Map<String, dynamic>> getPrinterInfo() async {
    final Map<String, dynamic> info = {};

    try {
      // Check if it's an Imin printer
      final isImin = await isIminPrinter();
      info['isIminPrinter'] = isImin;

      // Check if it's a built-in printer
      if (isImin == true) {
        final isBuiltIn = await isBuiltInPrinter();
        info['isBuiltInPrinter'] = isBuiltIn;
      } else {
        info['isBuiltInPrinter'] = false;
      }

      // Get basic printer information
      info['modelName'] = await getPrinterModelName();
      info['printModel'] = await getPrintModel();
      info['serialNumber'] = await getPrinterSerialNumber();
      info['sdkVersion'] = await getSdkVersion();

      // Get additional printer details if available
      try {
        info['thermalHead'] = await getPrinterThermalHead();
      } catch (_) {}

      try {
        info['firmwareVersion'] = await getPrinterFirmwareVersion();
      } catch (_) {}

      try {
        info['hardwareVersion'] = await getPrinterHardwareVersion();
      } catch (_) {}

      try {
        info['serviceVersion'] = await getServiceVersion();
      } catch (_) {}

      try {
        info['printerStatus'] = await getPrinterStatus();
      } catch (_) {}
      ...
      prints('Error getting printer info: $e');
  ...
  Future<void> printSingleBitmapColorChart(
    dynamic img, {
    IminPictureStyle? pictureStyle,
  }) {
    return IminPrinterPlatform.instance.printSingleBitmapColorChart(
      img,
      pictureStyle: pictureStyle,
    );
  }
  ...
  Future<void> labelInitCanvas({LabelCanvasStyle? labelCanvasStyle}) {
    return IminPrinterPlatform.instance.labelInitCanvas(
      labelCanvasStyle: labelCanvasStyle,
    );
  }

  Future<void> labelAddText(String text, {LabelTextStyle? labelTextStyle}) {
    return IminPrinterPlatform.instance.labelAddText(
      text,
      labelTextStyle: labelTextStyle,
    );
  }

  Future<void> labelAddBarCode(
    String barCode, {
    LabelBarCodeStyle? barCodeStyle,
  }) {
    return IminPrinterPlatform.instance.labelAddBarCode(
      barCode,
  ...
  Future<void> labelAddQrCode(String qrCode, {LabelQrCodeStyle? qrCodeStyle}) {
    return IminPrinterPlatform.instance.labelAddQrCode(
      qrCode,
      qrCodeStyle: qrCodeStyle,
    );
  }
  ...
  Future<void> labelAddBitmap(dynamic img, {LabelBitmapStyle? addBitmapStyle}) {
    return IminPrinterPlatform.instance.labelAddBitmap(
      img,
      addBitmapStyle: addBitmapStyle,
    );
  }
  ...
  Future<void> printLabelBitmap(
    ...
    return IminPrinterPlatform.instance.printLabelBitmap(
      img,
      printBitmapStyle: printBitmapStyle,
    );
  }
  ...
  Future<String?> getPrintModel() {
    return IminPrinterPlatform.instance.getPrintModel();
  }
}
