...
class DiscountItem extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DiscountItem> createState() => _DiscountItemState();
}

class _DiscountItemState extends ConsumerState<DiscountItem> {
  @override
  Widget build(BuildContext context) {
    final discountNotifier = ref.read(discountProvider.notifier);
    final isWithinRange = discountNotifier.isNowWithinRange(
      widget.discountModel.validFrom,
      widget.discountModel.validTo,
    );
  ...
  Column discountDetails(Color color) {
    return Column(
      ...
      children: [
        ...
        const Space(10),
        Text(
          widget.discountModel.name ?? 'noName'.tr(),
          style: AppTheme.normalTextStyle(color: color),
          maxLines: 2,
          textAlign: TextAlign.center,
        ),
        const Space(5),
        widget.discountModel.type == DiscountTypeEnum.amount
            ? Text(
              'RM'.tr(
                args: [
                  widget.discountModel.value?.toStringAsFixed(2) ??
                      0.toStringAsFixed(2),
                ],
              ),
              style: AppTheme.normalTextStyle(color: color),
            ...
            : Text(
              '${widget.discountModel.value?.toStringAsFixed(0)}%',
              style: AppTheme.normalTextStyle(color: color),
            ),
      ],
    );
