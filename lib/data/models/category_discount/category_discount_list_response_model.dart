...
class CategoryDiscountListResponseModel
    extends BaseAPIResponse<List<CategoryDiscountModel>, void> {
  CategoryDiscountListResponseModel(super.fullJson);

  @override
  List<Map<String, dynamic>>? dataToJson(List<CategoryDiscountModel>? data) {
    if (this.data != null) {
  ...
  @override
  List<CategoryDiscountModel>? jsonToData(Map<String, dynamic>? json) {
    if (json != null) {
      data = [];

      json['data'].forEach((v) {
        data!.add(CategoryDiscountModel.fromJson(v));
      });
