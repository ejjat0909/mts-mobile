...
class InventoryTransactionListResponseModel
    extends BaseAPIResponse<List<InventoryTransactionModel>, void> {
  InventoryTransactionListResponseModel(super.fullJson);

  @override
  List<Map<String, dynamic>>? dataToJson(
    List<InventoryTransactionModel>? data,
  ) {
    if (this.data != null) {
  ...
  @override
  List<InventoryTransactionModel>? jsonToData(Map<String, dynamic>? json) {
    if (json != null) {
      data = [];

      json['data'].forEach((v) {
        data!.add(InventoryTransactionModel.fromJson(v));
      });
